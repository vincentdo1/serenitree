...
import { getAllQuests } from '@/lib/quests' // Adjust the library or function to fetch quests

const getQuest = cache(async (id: string) => {
  let allQuests = await getAllQuests()
  let quest = allQuests.find((quest) => quest.id.toString() === id)
...
export async function generateMetadata({
  ...
  let quest = await getQuest(params.quest)
...
export default async function Quest({
  ...
  let quest = await getQuest(params.quest)
  let date = new Date(quest.published)
